	...
	switch(href_list["preference"])
		if("use_preset")
			...
			if(action && action == "Yes")
				...
				if(desired_set)
					...
					body_markings = assemble_body_markings_from_set(BMS, features, src)
		if("reset_color")
			...
			body_markings[zone][name] = BM.get_default_colors(features, src)
		if("change_color")
			var/zone = href_list["key"]
			var/name = href_list["name"]
			if(!body_markings[zone] || !body_markings[zone][name])
		...
		if("add_marking")
			...
			if(desired_marking)
				...
				body_markings[zone][BD.name] = BD.get_default_colors(features, pref_species)
		if("remove_marking")
			var/zone = href_list["key"]
			var/name = href_list["name"]
			if(!body_markings[zone] || !body_markings[zone][name])
		...
		if("change_marking")
			...
			if(desired_marking)
				...
				var/datum/body_marking/BD = GLOB.body_markings[desired_marking]
				var/marking_content
				marking_content = BD.get_default_colors(features, pref_species)
		...
		if("change_all_colors")
			var/new_color = color_pick_sanitized_lumi(user, "Choose your markings color:", "Character Preference")
			if(!new_color)
				return
			var/sanitised = sanitize_hexcolor(new_color, 6)
			for(var/zone in body_markings)
				var/markings = body_markings[zone]
				for(var/key in markings)
					markings[key] = sanitised
...
/datum/preferences/proc/print_body_markings_page()
	...
	dat += " <a href='?_src_=prefs;preference=reset_all_colors;task=change_marking'>Reset marking colors</a> |"
	dat += " <a href='?_src_=prefs;preference=change_all_colors;task=change_marking'>Set all marking colors</a>"
	dat += "<table width='100%'>"
	...
	for(var/zone in GLOB.marking_zones)
		...
		var/list/keys = body_markings[zone]

		for(var/key in keys)
			var/datum/body_marking/real_marking = GLOB.body_markings[key]
			var/can_move_up = null
			var/can_move_down = null
			var/color_line = null
			var/current_index = LAZYFIND(keys, key)
			var/color = keys[key]
			if(real_marking.allows_color_customisation)
				var/list/color_list = color_string_to_list(color)
				for(var/index in 1 to real_marking.color_keys)
					var/named_index = (real_marking.color_keys == 1) ? real_marking.color_key_name : real_marking.color_key_names[index]
					color_line = "<a href='?_src_=prefs;name=[key];key=[zone];preference=reset_color;task=change_marking'>R</a>"
					color_line += "<a href='?_src_=prefs;name=[key];key=[zone];index=[index];preference=change_color;task=change_marking'><span class='color_holder_box' style='background-color:[color_list[index]]'></span></a>"
			if(current_index < length(keys))
		...
		if(!length(keys) || length(keys) < MAXIMUM_MARKINGS_PER_LIMB)
			dat += "<tr style='vertical-align:top;'>"
...
/datum/preferences/proc/reset_body_marking_colors()
	for(var/zone in body_markings)
		...
		for(var/key in bml)
			...
			bml[key] = BM.get_default_colors(features, src)
