...
/datum/chatmessage
	...
	/// Fontsize in PT, 75% of size in PX
	var/font_size = 9
	/// Color override
	var/tgt_color
	/// Extra classes past maptext and centered
	var/list/extra_classes
	/// Text of the message
	var/text
	/// Strings to process
	...
	var/current_string = ""
	/// If we end after the current string
	var/premature_end = FALSE
	/// If this was an exclamation (! !!)
	var/exclaimed = FALSE
	/// List of blips for each string
	var/list/blip_tone = BLIP_TONE_DEFAULT
	/// If we shake when we speak
	var/source_shake = FALSE
	/// Cooldown on toot sounds
	COOLDOWN_DECLARE(toot_cooldown)
...
/datum/chatmessage/New(text, atom/target, mob/owner, datum/language/language, list/extra_classes = list(), lifespan = CHAT_MESSAGE_LIFESPAN)
	...
	src.extra_classes = extra_classes.Copy()
	...
	tgt_color = ("italics" in extra_classes) ? target.chat_color_darkened : target.chat_color
...
/datum/chatmessage/proc/turn_to_styled(string)
	return {"<span class='[length(extra_classes) ? extra_classes.Join(" ") : ""]' style='font-size:[font_size]pt; color:[tgt_color]'>[string]</span>"}
...
/datum/chatmessage/proc/_add_string(string = "", direction = 1, audible = TRUE)
	...
	message.maptext = MAPTEXT_CENTER(turn_to_styled(current_string))
	if(audible && !("emote" in extra_classes))
		/*
		play_toot()
...
/datum/chatmessage/proc/play_toot()
	if(!COOLDOWN_FINISHED(src, toot_cooldown))
		return

	COOLDOWN_START(src, toot_cooldown, TOOT_COOLDOWN)
...
/datum/chatmessage/proc/premature_end_of_life()
	...
	var/delay = rand(10, 20) * 0.1 DECISECONDS
